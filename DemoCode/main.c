...
static void set_swap_chain_size(int width, int height)
{
	...
	hresult = IDXGISwapChain_ResizeBuffers(swap_chain, 0, width, height, DXGI_FORMAT_UNKNOWN, 0);
	...
	hresult = IDXGISwapChain_GetBuffer(swap_chain, 0, &IID_ID3D11Texture2D, (void**)&back_buffer);
	assert(SUCCEEDED(hresult));

	hresult = ID3D11Device_CreateRenderTargetView(device, (ID3D11Resource*)back_buffer, &desc, &rt_view);
	assert(SUCCEEDED(hresult));
